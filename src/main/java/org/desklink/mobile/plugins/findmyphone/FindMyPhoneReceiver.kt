...
*/
package org.desklink.mobile.plugins.findmyphone
...
class FindMyPhoneReceiver : BroadcastReceiver() {
    companion object {
        const val ACTION_FOUND_IT: String = "org.desklink.mobile.plugins.findmyphone.foundIt"
    ...
    private fun foundIt(context: Context, intent: Intent) {
        ...
        val plugin = DeskLinkApplication.getInstance().getDevicePlugin(deviceId, FindMyPhonePlugin::class.java) ?: return
        plugin.stopPlaying()
        plugin.stopFlashing()
    }
}

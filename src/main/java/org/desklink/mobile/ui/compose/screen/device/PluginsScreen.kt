...
package org.desklink.mobile.ui.compose.screen.device
...
fun PluginsScreen(
    ...
    onRequiredPermissionClick: (plugin: Plugin) -> Unit,
    onOptionalPermissionClick: (plugin: Plugin) -> Unit
) {
    PluginsScreenContent(
        ...
        onRequiredPermissionClick = onRequiredPermissionClick,
        onOptionalPermissionClick = onOptionalPermissionClick
    )
}
        ...
        Column(modifier = Modifier.padding(top = 16.dp)) {
            ...
            if (pluginsNeedPermissions.isNotEmpty()) {
                PluginsWithoutPermissions(
                    ...
                    action = onRequiredPermissionClick
                )
            ...
            if (pluginsNeedOptionalPermissions.isNotEmpty()) {
                PluginsWithoutPermissions(
                    ...
                    action = onOptionalPermissionClick
                )
            }
        }
    }
...
private fun PluginsScreenPreview() {
    DeskLinkTheme(context = LocalContext.current) {
        ...
        PluginsScreenContent(
            ...
            onRequiredPermissionClick = { /* Do nothing */ },
            onOptionalPermissionClick = { /* Do nothing */ }
        )
    }
}
